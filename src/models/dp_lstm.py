"""
Differentially Private LSTM for BIST Trading Signals
    ...
    def train_epoch(self, 
        ...
        with BatchMemoryManager(
            ...
            for batch_idx, batch in enumerate(memory_safe_data_loader):
                ...
                # Compute loss
                loss, individual_losses = self.model.compute_loss(predictions, targets)
                ...
                if batch_idx % 50 == 0:
                    self.logger.debug(f"Batch {batch_idx}/{len(memory_safe_data_loader)}, "
                                    f"Loss: {loss.item():.4f}")
...
def create_sample_data(batch_size: int = 64, 
    ...
    # Training data
    train_x = torch.randn(batch_size * 10, sequence_length, input_size)
    train_direction = torch.randint(0, 3, (batch_size * 10,))  # 0=down, 1=neutral, 2=up
    train_magnitude = torch.randn(batch_size * 10) * 0.02  # Price change magnitude
    train_volatility = torch.abs(torch.randn(batch_size * 10)) * 0.01  # Volatility
    train_volume = torch.abs(torch.randn(batch_size * 10)) * 1000  # Volume
    
    # Validation data
    val_x = torch.randn(batch_size * 2, sequence_length, input_size)
    val_direction = torch.randint(0, 3, (batch_size * 2,))
    val_magnitude = torch.randn(batch_size * 2) * 0.02
    val_volatility = torch.abs(torch.randn(batch_size * 2)) * 0.01
    val_volume = torch.abs(torch.randn(batch_size * 2)) * 1000
...
def test_dp_lstm():
    ...
    print("🤖 Testing DP-LSTM Architecture...")
    print("=" * 60)

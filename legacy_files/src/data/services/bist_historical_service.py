...
class BISTHistoricalService:
        ...
        with self._lock:
            ...
            with self._get_connection() as conn:
                ...
                SELECT 
                    ...
                    p.prev_close
                FROM latest_data l
                LEFT JOIN stocks s ON l.symbol = s.symbol
                ...
                for row in conn.execute(query):
                    ...
                    # Use symbol as name if stocks table is empty
                    display_name = row['name'] if row['name'] and row['name'].strip() else row['symbol']
                    display_name_tr = row['name_turkish'] if row['name_turkish'] and row['name_turkish'].strip() else row['symbol']
                    display_sector = row['sector'] if row['sector'] and row['sector'].strip() else 'Unknown'
                    
                    stock = BISTHistoricalStock(
                        symbol=row['symbol'],
                        name=display_name,
                        name_turkish=display_name_tr,
    ...
    def get_historical_data(self, symbol: str, timeframe: str = "hourly", 
        ...
        with self._get_connection() as conn:
            query = """
            SELECT 
                date_time, timeframe, open_price, high_price, low_price, 
                close_price, volume, rsi_14, macd_line, macd_signal,
            ...
            data_points = []
            for row in conn.execute(query, (symbol.upper(), timeframe, limit)):

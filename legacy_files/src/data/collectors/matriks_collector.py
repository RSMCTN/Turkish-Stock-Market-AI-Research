    ...
    async def get_multiple_quotes(self, symbols: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        for symbol, task in tasks:
            try:
                ...
                self.logger.warning(f"Failed to fetch quote for {symbol}: {str(e)}")
        
        self.logger.info(f"Successfully fetched {len(results)} quotes")
        return results
...
async def test_connection(api_key: str) -> bool:
    """Test MatriksIQ API connection"""
    try:
        async with create_matriks_collector(api_key) as collector:
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        async with MatriksCollector(api_key) as collector:
            # Test health check
            health = await collector.health_check()
            print(f"Health check: {health}")
            
            # Test symbols (with mock key, won't work)
            if health:
                ...
                if symbols:
                    ...
                    df = await collector.get_historical_data(test_symbol, limit=10)
                    print(f"Historical data for {test_symbol}: {len(df)} rows")
